...
export function GeocellPageClient() {
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            '@type': 'ProductGroup',
            name: 'PCA Geocell Series',
            description: 'Professional PCA geocell systems for slope protection, road reinforcement, and channel lining.',
            brand: { '@type': 'Brand', name: 'SylvaGeo' },
            manufacturer: { '@type': 'Organization', name: 'SylvaGeo', url: 'https://www.sylvageo.com' },
            hasVariant: geocellProducts.map((p) => ({
              '@type': 'Product',
              name: p.name,
              description: `${p.name} - Cell height ${p.heights.join('/')}mm`,
            })),
          }),
        }}
      />
        ...
        <section id="downloads" className="py-16 border-t border-slate-rock-100 scroll-mt-32">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {downloadItemsComingSoon.map((item) => (
              <div
                key={item.name}
                className="bg-white border border-slate-rock-50 rounded-card shadow-card p-5 flex items-center gap-4 opacity-70"
              >
                <div className="w-10 h-10 bg-slate-rock-50 rounded-btn flex items-center justify-center flex-shrink-0">
                  <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-slate-rock-400">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                  </svg>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-rock-600 truncate">{item.name}</div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs bg-slate-rock-100 text-slate-rock-500 px-2 py-0.5 rounded-tag font-data">
                      {item.format}
                    </span>
                    <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-tag">
                      Coming Soon
                    </span>
                  </div>
                </div>
